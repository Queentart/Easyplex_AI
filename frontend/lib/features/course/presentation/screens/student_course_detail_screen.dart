...
/// Student course detail: course info header, then EVERY calendar day in the
/// course period (not only days with videos). Each day row is TAPPABLE and
/// opens the read-only day-detail (수업 일지 + 영상). Indicators show 일지 / 영상 N.
      ...
      itemBuilder: (context, index) {
        if (index == 0) {
          return Padding(
            ...
            child: _CourseHeaderCard(course: course),
          );
        }
...
/// A tappable day row → student read-only day-detail. Shows date + Korean
/// weekday and small indicators (일지 / 영상 N).
class _DayRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      onTap: () => context.push('/student/courses/$courseId/day/$yyyymmdd'),
